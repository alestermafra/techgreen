...
<nav class="navbar navbar-light">
	...
	<div>
		<a href="<?php echo $this->url('/diaria_velejador/inserir') ?>" class="btn btn-sm btn-success" role="button" title="Novo registro">Adicionar</a>
	</div>
</nav>
...
<div class="container-fluid">
	<div class="card">
		<div class="card-body py-2">
			<form action="<?php echo $this->url('/DiariaVelejador') ?>">	
				<div class="form-row">
					...
					<div class="form-group col-xl-2">
						...
						<select name="order" class="form-control form-control-sm" onchange="this.form.submit()">
							...
							<option value="tabela"<?php echo isset($_GET['order']) && $_GET['order'] === "tabela"? ' selected':'' ?>>Plano</option>
						</select>
					</div>
				</div>
			</form>
		...
		<table class="table table-sm table-hover">
			...
			<tbody>
				<?php foreach($list as $d): ?>
					<tr style="cursor: pointer" onclick="window.location = '<?php echo $this->url('/DiariaVelejador/view/' . $d['cdiaria']) ?>'">
	...
	<nav>
		<ul class="pagination pagination-sm justify-content-center">
			<li class="page-item <?php echo $page === 1? 'disabled':'' ?>">
				<a class="page-link" href="<?php echo $this->url('/DiariaVelejador' . $url_get . '&page=1') ?>">Primeira</a>
			</li>
			<li class="page-item <?php echo $page <= 1? 'disabled':'' ?>">
				<a class="page-link" href="<?php echo $this->url('/DiariaVelejador' . $url_get . '&page=' . ($page - 1)) ?>">Anterior</a>
			</li>
			<li class="page-item active"><a class="page-link" href="javascript:void(0)"><?php echo $page ?></a></li>
			<li class="page-item <?php echo ($pages === 0 || $page === $pages)? 'disabled':'' ?>"><a class="page-link" href="<?php echo $this->url('/DiariaVelejador' . $url_get . '&page=' . ($page + 1)) ?>">Próxima</a></li>
			<li class="page-item <?php echo ($pages === 0 || $page === $pages)? 'disabled':'' ?>"><a class="page-link" href="<?php echo $this->url('/DiariaVelejador' . $url_get . '&page=' . $pages) ?>">Última</a></li>
		</ul>
	</nav>
